...
def postprocess(output, model, threshold=0.3):
    ...
    for item in scores:
        ...
        for idx, s in enumerate(item):
            if s > threshold and model.config["id2label"][str(idx)] not in CLASSES_TO_IGNORE:
...
def get_task_user_agent_emotion(llm_input: str, llm_output: str, model, tokenizer) -> Dict[str, str]:
