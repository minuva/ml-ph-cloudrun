...
from .model import OnnxTransformer
...
def get_labels_above_threshold(toxicity: Dict[str, List[float]], threshold: float = 0.5) -> List[str]:
    return [label for label, probability in toxicity.items() if probability[0] >= threshold]


def get_task_user_agent_toxic(llm_input: str, llm_output, model: CustomDetoxify) -> Dict[str, Union[str, int]]:
